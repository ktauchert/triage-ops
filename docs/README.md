...
**TriageOps** (also referred to as **GitLab Lens**) is a developer-tooling platform that syncs GitLab issue metadata into a local Postgres database and surfaces triage metrics — ghost tickets, zombie tickets, milestone decay — via a dashboard.
...
| [Implementation Phases](./phases.md) | Phase 1 / Phase 2 roadmap with step-by-step plan |
...
npm test                   # Vitest (worker package)
```
...
triage-ops/
├── apps/
│   ├── web/          Next.js App Router dashboard (Shadcn UI planned)
│   └── worker/       BullMQ background jobs + GitLab API client
├── packages/
│   ├── db/           Prisma schema, migrations, Postgres client
│   └── shared-types/ Queue payloads and GitLab DTOs
├── docker-compose.yml
├── .env.example
└── docs/             ← you are here
```
...
| BullMQ sync worker | ✅ Core pipeline done |
| Vitest + MSW tests | ✅ GitLab client + locks |
| Web dashboard / API routes | ⏳ Not started |
